...
class ImdbSuiteTester extends AnyFunSuite with BeforeAndAfterAll {
   ...
   */
  def assertSameElements[A](givens: List[A], expected: List[A]): Unit = {
    val givenSet = givens.toSet
    ...
    val noMatchString =
      s"""
         ...
         |Actual:   ${previewList(givens)}""".stripMargin
    ...
    def assertSameElementsTask1(givens: List[(Float, Int, Int, String)], expected: List[(Float, Int, Int, String)]): Unit = {
      def round1(x: (Float, Int, Int, String)): (Int, Int, Int, String) = ((x._1 * 6).toInt, x._2, x._3, x._4)
      assertSameElements(givens.map(round1), expected.map(round1))
    }
    ...
    test("Task2") {
      assert(initializeImdb(), INIT_ERR_MSG)
      val rdd1 = ImdbAnalysis.titleBasicsRDD
      val rdd2 = ImdbAnalysis.titleRatingsRDD
      val res = ImdbAnalysis.task2(rdd1, rdd2).collect().toList
    ...
    test("Task3") {
      ...
      val rdd1 = ImdbAnalysis.titleBasicsRDD
      val rdd2 = ImdbAnalysis.titleRatingsRDD
      val res = ImdbAnalysis.task3(rdd1, rdd2).collect().toList
    ...
    test("Task4") {
      assert(initializeImdb(), INIT_ERR_MSG)
      val rdd1 = ImdbAnalysis.titleBasicsRDD
      val rdd2 = ImdbAnalysis.titleCrewRDD
      val rdd3 = ImdbAnalysis.nameBasicsRDD
      val res = ImdbAnalysis.task4(rdd1, rdd2, rdd3).collect().toList
      val expectedResult = List(
